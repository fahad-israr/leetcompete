    ...
    >
      ...
      <div style={{
        ...
        {messages.length === 0 ? (
          ...
          messages.map((m) => {
            if (m.type === 'SYSTEM' || m.type === 'SOLVE') {
              const isSolve = m.type === 'SOLVE';
              return (
                <div
                  key={m.id}
                  style={{
                    ...
                    background: isSolve ? 'rgba(16, 185, 129, 0.12)' : 'rgba(168, 85, 247, 0.08)',
                    border: `1px solid ${isSolve ? 'rgba(16, 185, 129, 0.3)' : 'rgba(168, 85, 247, 0.2)'}`,
                    fontSize: '0.825rem',
                    color: isSolve ? '#34d399' : '#d8b4fe',
                    lineHeight: 1.4,
                    display: 'flex',
                    alignItems: 'center',
                    gap: '6px'
            ...
            return (
              ...
              >
                <span style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginBottom: '2px', marginLeft: isMe ? 'auto' : '4px' }}>
                  {m.username}
                </span>
                <div
                  style={{
